...
static void add_hotkeys_to_compositor(GromitData *data) {
    ...
    if (xdg_current_desktop && strcmp(xdg_current_desktop, "GNOME") == 0) {
        ...
        modifier_hotkey_option[18] = data->select_color_keyval;
        modifier_hotkey_option[19] = "--select-color";
...
void setup_input_devices (GromitData *data)
{
    ...
    for(d = devices; d; d = d->next)
    {
        ...
        if (gdk_device_get_source(device) != GDK_SOURCE_KEYBOARD && gdk_device_get_n_axes(device) >= 2)
        {
            ...
            if (!data->hot_keycode && !data->undo_keycode && !data->select_color_keycode)
            {
            ...
            if (GDK_IS_X11_DISPLAY(data->display)) {
                ...
                devinfo = XIQueryDevice(GDK_DISPLAY_XDISPLAY(data->display), dev_id, &devicecount);
                if (devicecount) {
                ...
                {
                    if(data->debug)
                        g_printerr("DEBUG: Grabbing hotkeys '%s', '%s' and '%s' from keyboard '%d' .\n", data->hot_keyval, data->undo_keyval, data->select_color_keyval, kbd_dev_id);


                    setup_shortcut(data, data->hot_keyval, data->hot_keycode, kbd_dev_id);
                    setup_shortcut(data, data->undo_keyval, data->undo_keycode, kbd_dev_id);
                    setup_shortcut(data, data->select_color_keyval, data->select_color_keycode, kbd_dev_id);
...
void setup_shortcut(GromitData *data, gchar* keyval, guint keycode, gint kbd_dev_id) {
    XIEventMask mask;
        ...
        gpointer     func_data)
{
    ...
    if (event->type == GDK_KEY_PRESS && event->hardware_keycode == data->hot_keycode)
    {
        if(data->debug)
    ...
    if (event->type == GDK_KEY_PRESS && event->hardware_keycode == data->undo_keycode)
    {
        if(data->debug)
    ...
    if (event->type == GDK_KEY_PRESS && event->hardware_keycode == data->select_color_keycode)
    {
        if(data->debug)
            g_printerr("DEBUG: Received undokey press from device '%s'\n", gdk_device_get_name(dev));
